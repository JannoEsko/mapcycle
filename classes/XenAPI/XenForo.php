...
class XenForo {
    ...
    static $SQL_QUERIES = array(
        ...
        , "GET_USER_DATA_BY_USER_ID" => "SELECT user_id, username, user_group_id FROM xf_user WHERE user_id = ?"
    ...
    static function getUserDataByUsernameEmail(SQL $sql, $username_email) {
        $user_data = $sql->query(self::$SQL_QUERIES['GET_USER_DATA_BY_USERNAME_EMAIL'], array($username, $username));
        if (sizeof($user_data) === 1) {
            $user_data = $user_data[0];
        } else {
        ...
        return array("name" => $user_data['username'], "member_id" => intval($user_data['user_id']), "member_group_id" => intval($user_data['user_group_id']));
    }
    
    static function getUserDataByID(SQL $sql, $member_id) {
        $member_id = intval($member_id);
        if ($member_id <= 0) {
            throw new LogicException("given member_id below/equal to 0");
        }
        $user_data = $sql->query(self::$SQL_QUERIES['GET_USER_DATA_BY_USER_ID'], array($member_id));
        if (sizeof($user_data) === 1) {
            $user_data = $user_data[0];
        } else {
            return false;

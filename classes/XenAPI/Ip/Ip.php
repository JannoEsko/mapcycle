...
/**
 * Taken from XenForo2 Utils source.
 */
...
class Ip {
    ...
    public static function convertIpHexToString($ip) {
        if (substr($ip, 0, 2) === "0x") {
            return self::convertIpBinaryToString(base_convert($ip, 16, 2));
            $ip = substr($ip, 2);
            
        } else {
            return self::convertIpBinaryToString($ip);
        }
        $ip = self::convertHexToBin($ip);
        return self::convertIpBinaryToString($ip);
    }

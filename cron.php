...
Statistics::parseServerpanelStats(Constants::getSQL());
    ...
    if (intval($currlive->getPushtolive_id()) === intval($potentialLive->getPushtolive_id())) {
        if ($currlive->isLive()) {
            ...
            try {
                ...
                XenApi::addPost("Resent mc (id " . $currlive->getMapcycle_id() . ") to live.", 8, 1);
            } catch (Exception $ex) {
                
                XenApi::addPost("Exception on CRON (fail to resend to Live)\r\n" . $ex->getMessage() . "\r\nFile - " . $ex->getFile() . " (" . $ex->getLine() . ")\r\n\r\nFull trace: " . $ex->getTraceAsString(), 8, 1);
            }
        ...
        try {
            ...
            XenApi::addPost("Pushed new mc (id " . $potentialLive->getMapcycle_id() . ") to live.", 8, 1);
        } catch (Exception $ex) {
            $currlive->setLive(1)->write()->pushToProd();
...
if ($currlive === null || !($currlive instanceof PushToLive)) {
    try {
        $potentialLive->setLive(1)->write()->pushToProd();
        XenApi::addPost("Currlive = null, Pushed new mc (id " . $potentialLive->getMapcycle_id() . ") to live.", 8, 1);
    } catch (Exception $ex) {

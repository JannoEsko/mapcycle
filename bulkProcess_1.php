...
$data = $sql->query("SELECT map_entity FROM maps_entities");
...
$rii = new RecursiveIteratorIterator(new RecursiveDirectoryIterator(__DIR__ . "/blknew"));
$up_by = 1;
$up_by_ip = "127.0.0.1";
...
foreach ($rii as $inp) {
    ...
    if (strtolower($inp->getExtension()) === "ent") {
        ...
        $mapDesc = $inp->getPathname() . " (from Magikzz / Demobase leak)";
        ...
        if (!in_array($tmp, $allEntities)) {
            ...
            $j++;
            echo "Map $mapName not duplicate\n";
        } else {
            echo "Map $mapName duplicate.\n";
        }
        $i++;
    }
...
echo "$i maps processed. $j maps uploaded. \n";
